...
procedure TFormCadastroPai.btnNovoClick(Sender: TObject);  //botãoNovo
begin
  if not fdQryCadastro.Active then begin
    Self.SetRecord(0, tNil);
  end;

  if not (fdQryCadastro.State in [dsEdit, dsInsert]) then begin     //condicção pra se não estiver editando ou já inserido, ele vai inserir
    fdQryCadastro.Insert;   //inserindo
  end;

end;

procedure TFormCadastroPai.btnGravarClick(Sender: TObject);      //botãoGravar
begin
  if fdQryCadastro.State in [dsEdit, dsInsert] then begin  //se estiver em modo de edição ou inserido, ele irá gravar
    fdTransaction.StartTransaction;
    fdQryCadastro.Post;     //gravando
    fdTransaction.CommitRetaining;     //commitando
    MsgInformacao('Gravado com sucesso!');
  end;

end;

procedure TFormCadastroPai.btnCancelarClick(Sender: TObject);   //botãoCancelar
begin
  if fdQryCadastro.State in [dsEdit, dsInsert] then begin
      if MsgPerguntar('Tem certeza que deseja cancelar?', False) then begin
       fdTransaction.StartTransaction;
       fdQryCadastro.Cancel;               //cancelando
       fdTransaction.RollbackRetaining;     //desfazendo
     end;
  end;
end;

procedure TFormCadastroPai.btnExcluirClick(Sender: TObject);      //botãoExcluir
begin
